  ...
  #      return @dw.get_emissions_info(new_word, ['NC*','A*','VP*','PQMS']) if new_word != word
   ...
   #   end
      ...
      if word =~ /^super/
        ...
        return @tagger_config.dw.get_emissions_info(new_word, ['A*','W','VP*']) if new_word != word
      end
      ...
      if word =~ /^hiper/
        ...
        return @tagger_config.dw.get_emissions_info(new_word, ['A*','W']) if new_word != word
      end
      ...
      if word =~ /bilísim[oa]s?$/
        ...
        return @tagger_config.dw.get_emissions_info(new_word, ['AF*']) if word =~ /as?$/
        return @tagger_config.dw.get_emissions_info(new_word, ['AM*','AN*','AA*']) if word =~ /os?$/

      end
      # riquísimo => rico
      if word =~ /quísim[oa]s?$/
        new_word = word.gsub(/bilísim([oa]s?$)/,'c\1')
        return @tagger_config.dw.get_emissions_info(new_word, ['A*'])
      ...
      if word =~ /güísim[oa]s?$/
        ...
        result = @tagger_config.dw.get_emissions_info(new_word, ['A*'])
        if result.empty?
      ...
      if word =~ /lísim[oa]s?$/
        ...
        StringUtils.tilde_combinations(new_word).each do |combination|
          result = @tagger_config.dw.get_emissions_info(combination, ['AF*']) if word =~/as?$/
          result = @tagger_config.dw.get_emissions_info(combination, ['AM*','AN*','AA*']) if word =~ /os?$/
      ...
      if word =~ /císim[oa]s?$/
        ...
        result = @tagger_config.dw.get_emissions_info(new_word, ['AF*']) if word =~ /as?$/
        result = @tagger_config.dw.get_emissions_info(new_word, ['AM*','AN*','AA*']) if word =~ /os?$/
        return result unless result.empty?
      ...
      if word =~ /it[oa]s?$/ and word !~ /[-\/]/
        if word =~ /guit[oa]s?$/
          ...
          result = gender_number_force_matching(word, @tagger_config.dw.get_emissions_info(new_word, ['NC*','A*','VP*','VGP','W','DI*']))
          return result unless result.empty?
          if new_word !~/áéíóú/
        ...
        if word =~ /quit[oa]s?$/ and word != "quito"
          ...
          result = gender_number_force_matching(word, @tagger_config.dw.get_emissions_info(new_word, ['NC*','A*','VP*','VGP','W','DI*']))
          return result unless result.empty?
          result = gender_number_force_matching(word, @tagger_config.dw.get_emissions_info(new_word, ['VI*','VS*','VMP*']))
          unless result.empty?
            if new_word !~ /áéíóú/
              new_word_2 = set_tilde(new_word, 3) # Maybe this doesn't cover all the casuistic
              # musiquita => música / musiquito => músico
              result = @tagger_config.dw.get_emissions_info(new_word_2, ['NC*','A*','VP*','VGP','W','DI*'])
              return result unless result.empty?
            end
          end
          if new_word !~ /áéíóú/
            new_word_2 = set_tilde(new_word, 3) # Maybe this doesn't cover all the casuistic
            result = gender_number_force_matching(word, @tagger_config.dw.get_emissions_info(new_word_2, ['NC*','A*','VP*','VGP','W','DI*']))
        ...
        if word =~ /ncit[oa]s?$/
          ...
          if word =~ /oncitas?$/
            ...
            result = gender_number_force_matching(word, @tagger_config.dw.get_emissions_info(new_word, ['NCF*','AF*']))
            return result unless result.empty?
        ...
        if word =~ /rcit[oa]s?$/
          ...
          result = gender_number_force_matching(word, @tagger_config.dw.get_emissions_info(new_word, ['N*','A*']))
          return result unless result.empty?
          ...
          if word =~ /rcit[oa]$/
            ...
            result = gender_number_force_matching(word, @tagger_config.dw.get_emissions_info(new_word, ['A*','N*']))
            return result unless result.empty?
          end
          if word =~ /rcit[oa]s$/
        ...
        if word =~ /[fmvj]it[oa]s?$/
          ...
          result = gender_number_force_matching(word, @tagger_config.dw.get_emissions_info(new_word, ['N*','A*','DI*']))
          return result unless result.empty?
          # jefito => xefe
          # suavitas => suaves
        ...
        if word =~ /tit[oa]s?$/
          ...
          result = gender_number_force_matching(word, @tagger_config.dw.get_emissions_info(new_word, ['N*','A*','W','DI*']))
          return result unless result.empty?
          # tomatitos => tomates
          # amantitas => amantes
          new_word = word.gsub(/tit[oa](s?)$/,'te\1')
          result = gender_number_force_matching(word, @tagger_config.dw.get_emissions_info(new_word, ['N*','A*','VP*']))
          return result unless result.empty?
          # disgustito
        ...
        if word =~ /dit[oa]s?$/ and word !~ /ndit[oa]s?$/
          ...
          result = gender_number_force_matching(word, @tagger_config.dw.get_emissions_info(new_word, ['N*','A*','DI*','VP*']))
          return result unless result.empty?
        end
        if word =~ /dit[oa]s?$/ and word !~ /ndit[oa]s?$/
          # humildito => humilde
