...
class BasicSuffixes
  ...
  def initialize(empty_word, max_suffix_length, max_occurrences, words, tags_info, memmory)
  ...
  def calculate_frequencies
    @words.frequencies.each do |key, freq|
      ...
      if @memmory
        ...
        if (word != @empty_word) and (@words.get_frequency(word,tag).to_i <= @max_occurrences)
          add_word(word, tag, freq.to_i)
        end
      end
    end
  end
